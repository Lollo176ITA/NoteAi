...
fun AiStatusBanner(
    ...
    AnimatedVisibility(
        ...
        when (status) {
            AiStatus.Idle -> Unit
            is AiStatus.Pending -> BannerSurface(
                tone = BannerTone.Info,
                content = {
                    Text(
                        text = if (status.secondsLeft > 0)
                            stringResource(R.string.ai_status_pending, status.secondsLeft)
                        else
                            stringResource(R.string.ai_status_running),
